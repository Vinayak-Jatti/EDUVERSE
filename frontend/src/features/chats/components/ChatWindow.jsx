...
/**
 * 16. Create ChatWindow component — displays messages for selected room
 */
const ChatWindow = ({ 
  room, 
  messages, 
  ...
  // Anchor scroll to bottom ONLY when a new message arrives at the end of the array
  ...
  }, [lastMessageId]); // Tracking ONLY the latest message ID strictly prevents scroll-snapping on history

  // 23. Paginate message history on scroll-up
  const handleScroll = (e) => {
    if (e.target.scrollTop === 0 && onLoadOlderMessages && !isLoadingOlder) {
      onLoadOlderMessages();
    }
  };
    ...
    <div className="flex flex-col h-full bg-[#FBFBFE] overflow-hidden lg:rounded-r-[3rem] relative">
      ...
      <div className="h-16 md:h-20 bg-white/80 backdrop-blur-xl border-b border-black/5 flex items-center justify-between px-4 md:px-8 shrink-0 relative z-20">
        ...
        <div className="flex items-center gap-1 md:gap-2">
            <button className="p-2 md:p-3 text-gray-300 hover:text-black hover:bg-gray-50 rounded-xl transition-all hidden sm:block" title="Video call (Phase 2)"><Video size={18} /></button>
      ...
      >
        ...
        <AnimatePresence mode="popLayout">
          {messages.map((msg, idx) => {
            ...
            // 24. Visually distinguish AI messages from user messages (badge or color)
              ...
              >
                  {!isMe && (
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg md:rounded-xl overflow-hidden border border-black/5 mt-auto mr-2 md:mr-3 shadow-sm shrink-0 hidden xs:block bg-gray-100 flex-shrink-0 flex items-center justify-center">
                         {isAI ? (
                            ...
                            <img src={msg.sender_avatar || `https://api.dicebear.com/7.x/shapes/svg?seed=${msg.sender_id}`} className="w-full h-full object-cover" alt={msg.sender_name} />
                         )}
                      </div>
                  )}
                  ...
                  <div className={`max-w-[85%] md:max-w-[70%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-1.5`}>
                      {isCommunity && !isMe && !isAI && (
                          <span className="text-[10px] font-bold text-gray-500 ml-1">{msg.sender_name}</span>
                      )}
...
const WelcomePlaceholder = () => (
    <div className="h-full flex flex-col items-center justify-center p-20 text-center bg-gray-50/20 backdrop-blur-3xl animate-pulse-slow">
        ...
        <h2 className="text-3xl font-black uppercase tracking-tighter mb-4 leading-none">EduVerse <br/>Secure Comms</h2>
